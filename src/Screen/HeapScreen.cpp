...
inputVal = new InputBox(150, 80, 100, 40, font, 3);
    btnInsert = new Button(270, 80, 100, 40, font, "Insert");
    btnSearch = new Button(390, 80, 100, 40, font, "Search");
    btnDeleteMax = new Button(510, 80, 100, 40, font, "Delete"); 
    btnInit = new Button(630, 80, 100, 40, font, "Init");
    btnInitFromFile = new Button(750, 80, 100, 40, font, "Init File");
    btnUpdate = new Button(870, 80, 100, 40, font, "Update"); // Đưa Update về cuối

    // --- HÀNG 2 (y = 140): Playback & Popups ---
    btnSpeedDown   = new Button(150, 140, 50, 40, font, "<<");
    btnSkipBack    = new Button(190 + 20, 140, 40, 40, font, "|<"); 
    btnStepBack    = new Button(240 + 20, 140, 40, 40, font, "<");
    btnPausePlay   = new Button(290 + 20, 140, 120, 40, font, "Pause/Play");
    btnStepForward = new Button(420 + 20, 140, 40, 40, font, ">");
    btnSkipForward = new Button(470 + 20, 140, 40, 40, font, ">|"); 
    btnSpeedUp     = new Button(520 + 20, 140, 50, 40, font, ">>");
    ...
    inputUpdateVal = new InputBox(870, 140, 60, 40, font, 3); 
    btnGoUpdate = new Button(940, 140, 50, 40, font, "Go");
...
HeapScreen::~HeapScreen() {
    ...
    // Thêm dòng này vào trong HeapScreen::~HeapScreen()
...
void HeapScreen::handleEvent(sf::Event& event, sf::RenderWindow& window) {
    ...
    if (showUpdateInput) inputUpdateVal->handleEvent(event, mousePosUI);

    // Bật tắt Popup Update
    if (btnUpdate->isClicked(event, mousePosUI)) {
        showUpdateInput = !showUpdateInput;
    }

    // Xử lý khi nhấn nút Go của Update
    if (showUpdateInput && btnGoUpdate->isClicked(event, mousePosUI)) {
        ...
        if (!newText.empty() && !oldText.empty()) {
            try {
                ...
                heapTree->updateVal(oldVal, newVal);
                inputVal->clear();
...
void HeapScreen::update(float deltaTime, sf::RenderWindow& window) {
    ...
    // 1. Trong hàm HeapScreen::update(...) thêm:
    btnUpdate->update(mousePosUI);
    if (showUpdateInput) {
        btnGoUpdate->update(mousePosUI);
    }
